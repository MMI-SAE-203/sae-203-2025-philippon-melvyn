...
export const InviterService = {
    ...
    async getFilms(inviteId) {
        const invite = await pb.collection("Invite").getOne(inviteId);
        const films = invite?.associe_film;
        return films ? await pb.collection("Film").getFullList({ filter: `id IN [${films.join(",")}]` }) : [];
    },

    async getActivities(inviteId) {
        const invite = await pb.collection("Invite").getOne(inviteId);
        const activites = invite?.associe_activite;
        return activites ? await pb.collection("Activites").getFullList({ filter: `id IN [${activites.join(",")}]` }) : [];
    }
};
